...
\noindent Pre-registration, data and code: \url{https://github.com/panaum/llm-redteam-suite}, branch \code{judge-validation}.
...
\begin{itemize}
  \item \textbf{Models:} 137 of 197 items (70\%) come from \code{llama-3.1-8b-instant}, 26 from \code{llama-3.3-70b-versatile}, 22 from \code{qwen/qwen3-32b} and 12 from \code{openai/gpt-oss-120b}.
...
\paragraph{By model (indicative only).} Embedding $\kappa$ was 0.163 [0.029, 0.294] on the 137 \code{llama-3.1-8b-instant} items and 0.072 $[-0.102, 0.261]$ on the 60 items from the other three models.
...
\paragraph{Sample composition.} 70\% of the items come from one small model, \code{llama-3.1-8b-instant}. That model was also the LLM judge inside the stored composite, so for static rows it judged its own responses. The results describe judge behaviour mainly on that model's response style.
